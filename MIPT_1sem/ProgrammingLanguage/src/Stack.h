...
#ifndef PROGRAMMINGLANGUAGE_STACK_H
#define PROGRAMMINGLANGUAGE_STACK_H
...
const size_t limitedSizeOfString = 31;
const size_t DEFAULT_STACK_SIZE = 16;


struct codeblock {
    explicit codeblock(char* begin) : string(begin) {};
    codeblock() = default;
...
public:
    ...
    Stack(size_t new_capacity):
            ...
            size     (0),
            data     (new codeblock[new_capacity]) {
        fill();
    }
    ...
    */
    ...
    inline codeblock get() { return at(counter++); }
    ...
    [[nodiscard]] inline codeblock*      Get_Data()      const;
...
inline
bool Stack::pop(codeblock* element){
...
inline
bool Stack::push(codeblock item){
    if(!this->Stack_OK()) return false;
...
inline
codeblock& Stack::operator[](size_t index){
...
void Stack::Expand(bool* success) {
    (capacity*=2)++;
    data = (codeblock*) realloc(data, capacity * sizeof(codeblock));
...
inline
codeblock* Stack::Get_Data() const { return data; }

inline
const char* Stack::Get_Name() const { return name; }

inline
codeblock& Stack::at(int index) const {
    return *(data + index);
}
...
inline bool operator==(const codeblock& ss1, const codeblock& ss2) {
...
inline int Arrange_str_ptrs(codeblock* pointers, size_t number_of_lines, char* text) {
...
#endif //PROGRAMMINGLANGUAGE_STACK_H
